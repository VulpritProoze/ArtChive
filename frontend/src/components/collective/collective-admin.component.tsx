...
import {
  ...
  type AdminRequest,
  type JoinRequest,
} from "@hooks/queries/use-collective-data";
...
const getRequesterFullName = (request: AdminRequest | JoinRequest) => {
...
export default function CollectiveAdmin() {
  ...
  const activeTab = (() => {
    ...
    if (tab === 'membership-requests' || tab === 'admin-requests') {
      return tab as "membership-requests" | "admin-requests";
    }
    return "members" as const;
  })();

  const setActiveTab = (tab: "members" | "membership-requests" | "admin-requests") => {
    if (tab === "members") {
  ...
  const { data: adminRequests = [], isLoading: loadingAdminRequests } = useAdminRequests(
    collectiveId,
    { enabled: activeTab === 'admin-requests' }
  );
  const { data: joinRequests = [], isLoading: loadingJoinRequests } = useJoinRequests(
    collectiveId,
    { enabled: activeTab === 'membership-requests' }
  );

  // Mutations
  const processAdminRequestMutation = useProcessAdminRequest();
  const processJoinRequestMutation = useProcessJoinRequest();
  ...
  const handleKickMember = async (memberId: number, username: string) => {
    ...
    try {
      await kickMemberMutation.mutateAsync({ collectiveId: collectiveId!, memberId });
  ...
  const handlePromoteToAdmin = async (memberId: number, username: string) => {
    ...
    try {
      await promoteMemberMutation.mutateAsync({ collectiveId: collectiveId!, memberId });
  ...
  const handleDemoteMember = async (memberId: number, username: string) => {
    ...
    try {
      await demoteMemberMutation.mutateAsync({ collectiveId: collectiveId!, memberId });
  ...
  const handleAdminRequest = async (
    ...
    try {
      await processAdminRequestMutation.mutateAsync({ requestId, action, collectiveId: collectiveId! });
      toast.success(
        'Request processed',
  ...
  const handleJoinRequest = async (
    ...
    try {
      await processJoinRequestMutation.mutateAsync({ requestId, action, collectiveId: collectiveId! });
      toast.success(
        'Request processed',
        `Join request from ${username} has been ${action}d successfully.`
      );
    } catch (error) {
    ...
    <MainLayout showRightSidebar={false}>
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="tabs tabs-boxed mb-6">
          ...
          <a
            className={`tab ${activeTab === "membership-requests" ? "tab-active" : ""}`}
          ...
          <a
            className={`tab ${activeTab === "admin-requests" ? "tab-active" : ""}`}
        ...
        {activeTab === "membership-requests" && (
          <div>
            ...
            <div className="alert alert-info mb-6">
              ...
              <span>
                Review and approve/reject membership requests from users who want to join this collective.
              </span>
            </div>
